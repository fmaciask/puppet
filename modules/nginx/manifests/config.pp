...
#  }
  ...
  file { "${nginx::params::nx_conf_dir}/nginx.conf":
    ...
    content => template('/etc/puppet/modules/nginx/templates/conf.d/nginx.conf.erb'),
  }
...
# incluyo el archivo de bloqueo
  file { "${nginx::params::nx_conf_dir}/block.conf":
	ensure  => file,
	source => '/etc/puppet/modules/nginx/files/block.conf',
  }
}
